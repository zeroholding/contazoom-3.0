...
interface FaturamentoPorExposicaoProps {
...
const CORES = {
  "Premium": "#8b5cf6", // Roxo
  "Clássico": "#f59e0b", // Laranja
};

export default function FaturamentoPorExposicao({
  periodoAtivo = "todos",
  ...
  refreshKey = 0,
  selectedAccount,
}: FaturamentoPorExposicaoProps) {
  const [dados, setDados] = useState<DadosExposicao[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // Chamar API para dados do faturamento por exposição
        const url = `/api/dashboard/faturamento-por-exposicao${params.toString() ? `?${params.toString()}` : ''}`;
        ...
        const data = (await res.json()) as DadosExposicao[];
        
        // Debug: Log dos dados recebidos
        console.log('[FaturamentoPorExposicao] Dados recebidos da API:', data);
        console.log('[FaturamentoPorExposicao] Quantidade de tipos:', data.length);
        
        if (isMounted) {
          setDados(data);
        }
      } catch (err) {
        console.error("Falha ao carregar faturamento por exposição:", err);
        if (isMounted) {
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          <p className="font-medium text-gray-900 mb-2">{data.exposicao}</p>
    ...
    return (
      <div className="bg-[#F3F3F3] rounded-lg border border-gray-200 p-3 shadow-sm">
        <div className="flex items-center mb-4">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-800">Faturamento por Exposição</h3>
    ...
    <div className="bg-[#F3F3F3] rounded-lg border border-gray-200 p-3 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-800">Faturamento por Exposição</h3>
            <p className="text-xs text-gray-500">Distribuição entre Premium e Clássico</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="text-sm text-gray-600">
            Mostrando faturamento por exposição
          </div>
        </div>
      </div>
          ...
          <PieChart>
            ...
            >
              {dados.map((entry, index) => (
                <Cell 
                  key={`cell-${index}`} 
                  fill={CORES[entry.exposicao as keyof typeof CORES] || "#6b7280"} 
                />
              ))}
            </Pie>
